...
void EdgeReprojectionIntri::ComputeResidual() {
//    std::cout << pts_i_.transpose() <<" "<<pts_j_.transpose()  <<std::endl;
    ...
    Vec3 ptsi, ptsj; 
    ptsi.x() = (pts_i_.x() - cxi)/fi;
    ptsi.y() = (pts_i_.y() - cyi)/fi; 
    ptsi.z() = 1; 

    ptsj.x() = (pts_j_.x() - cxj)/fj; 
    ptsj.y() = (pts_j_.y() - cyj)/fj; 
    ptsj.z() = 1;
...
void EdgeReprojectionIntri::ComputeJacobians() {
    double inv_dep_i = verticies_[0]->Parameters()[0];
...
//    reduce = information_ * reduce;
    ...
    Eigen::Matrix<double, 2, 9> jacobian_pose_i;
    Eigen::Matrix<double, 3, 9> jaco_i;
    jaco_i.leftCols<3>() = ric.transpose() * Rj.transpose(); // dpj'_dti
    jaco_i.block<3, 3>(0,3) = ric.transpose() * Rj.transpose() * Ri * -Sophus::SO3d::hat(pts_imu_i); // dpj'_dqi 
    jaco_i.rightCols<3>() = ric.transpose() * Rj.transpose() * Ri * ric * dpi_din / (inv_dep_i); // dpj'_din_i
    jacobian_pose_i.leftCols<9>() = reduce * jaco_i;

    Eigen::Matrix<double, 2, 9> jacobian_pose_j;
    Eigen::Matrix<double, 3, 6> jaco_j;
    jaco_j.leftCols<3>() = ric.transpose() * -Rj.transpose(); // dpj'_dtj
