...
void EdgeImuIntric::ComputeResidual() {
    VecX param_0 = verticies_[0]->Parameters();
    Qd Qi(param_0[6], param_0[3], param_0[4], param_0[5]);
    Vec3 Pi = param_0.head<3>();
...
void EdgeImuIntric::ComputeJacobians() {
...
//    if (jacobians[0])
    {
        Eigen::Matrix<double, 15, 9, Eigen::RowMajor> jacobian_pose_i;
        jacobian_pose_i.setZero();
...
//    if (jacobians[2])
    {
        Eigen::Matrix<double, 15, 9, Eigen::RowMajor> jacobian_pose_j;
        jacobian_pose_j.setZero();
