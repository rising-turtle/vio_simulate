...
void System::PubFeatureData(double dStampSec, const vector<int> ids, /*const vector<Vector3d> landmarks,*/ const vector<Vector2d> feature_positions_un,
 const vector<Vector2d> feature_positions, const vector<Vector2d> feature_velocities)
{
	shared_ptr<IMG_MSG> feature_points(new IMG_MSG());
        feature_points->header = dStampSec;
        vector<set<int>> hash_ids(NUM_OF_CAM);
		...
		int p_id = ids[j];
                hash_ids[i].insert(p_id);
                double x = feature_positions_un[j].x();
                double y = feature_positions_un[j].y();
                double z = 1;//Normalized plane
                feature_points->points.push_back(Vector3d(x, y, z));
                feature_points->id_of_point.push_back(p_id * NUM_OF_CAM + i);
                feature_points->u_of_point.push_back(feature_positions[j].x());
                feature_points->v_of_point.push_back(feature_positions[j].y());
                feature_points->velocity_x_of_point.push_back(feature_velocities[j].x());//speed of movement of feature points in pixels
                //-> handle the problem of timestep alignment within imu and images(Not be considered in this HW)
                feature_points->velocity_y_of_point.push_back(feature_velocities[j].y());
