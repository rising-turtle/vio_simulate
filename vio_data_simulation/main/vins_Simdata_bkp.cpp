...
//    std::cout << Qwb.coeffs().transpose() <<"\n"<<Qwb.toRotationMatrix() << std::endl;
    ...
    // points obs in image
    //Added for HW 7
    //timestamp for cameras
    std::stringstream filename2;
    filename2<<"PointFrame/Timestamp.txt";
    ...
    for(int n = 0; n < camdata.size(); ++n)
    {
        ...
        for (int i = 0; i < points.size(); ++i) {
           ...
           if( (obs(0)*460 + 240) < params.image_h && ( obs(0) * 460 + 240) > 0 &&
                   (obs(1)*460 + 320) > 0 && ( obs(1)* 460 + 320) < params.image_w )
            {
        ...
        filename1<<"PointFrame/all_points_"<<n<<".txt";
...
//        std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d> > points_cam;    // ３维点在当前cam视野里
        ...
        // save lines
        std::stringstream filename1;
        filename1<<"PointFrame/all_lines_"<<n<<".txt";
