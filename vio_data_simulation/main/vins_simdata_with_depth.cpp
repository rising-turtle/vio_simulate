//
// Created by He Zhang on Nov. 26, 2019, generate feature with depth 
//
...
//    std::cout << Qwb.coeffs().transpose() <<"\n"<<Qwb.toRotationMatrix() << std::endl;
    ...
    for(int n = 0; n < camdata.size(); ++n)
    {
        ...
        std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > features_cam;
        for (int i = 0; i < points.size(); ++i) {
