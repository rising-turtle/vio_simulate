...
void IMU::testImu_Euler(std::string src, std::string dist)
{
    ...
    std::cout<<"test　testImu_Euler end"<<std::endl;

}


void IMU::testImu_MidPoint(std::string src, std::string dist)
{
    std::vector<MotionData>imudata;
    LoadPose(src,imudata);
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        // 中值积分 (作业)
        MotionData imupose_pre = imudata[i-1]; // i 从1开始 所以不需要考虑溢出了
        MotionData imupose_cur = imudata[i];

        Eigen::Vector3d dtheta_half_pre = 0.5*imupose_pre.imu_gyro * dt;
        Eigen::Vector3d dtheta_half_cur = 0.5*imupose_cur.imu_gyro * dt;
        Eigen::Vector3d dtheta_half = 0.5*(dtheta_half_cur + dtheta_half_pre);
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        Eigen::Vector3d acc_w_pre=Qwb * (imupose_pre.imu_acc)+gw; 

        Qwb= Qwb * dq;
        // std::cout << (Qwb * (imupose_cur.imu_acc)+gw)<<std::endl;
        Qwb.normalize();
        // std::cout << (Qwb * (imupose_cur.imu_acc)+gw)<<std::endl<<std::endl;
        Eigen::Vector3d acc_w_cur=Qwb * (imupose_cur.imu_acc)+gw; 
        Eigen::Vector3d acc_w=0.5*(acc_w_pre+acc_w_cur);
    ...
    std::cout<<"test　testImu_MidPoint end"<<std::endl;
